...
export function setOwnerNumber(n) {
  // Nettoyer : retirer @s.whatsapp.net, :device suffix, et tout non-chiffre
  ownerNumber = String(n).replace(/@.*$/, '').replace(/:\d+$/, '').replace(/\D/g, '');
  log('info', `Numéro propriétaire défini : ${ownerNumber}`);
}
...
async function handleIncomingMessages(sock, { messages }) {
  for (const msg of messages) {
    ...
    // Numéro expéditeur : pour les messages "fromMe" dans un chat solo,
    // le JID correspond au numéro du propriétaire lui-même
    const senderRaw = msg.key.fromMe
      ? (sock.user?.id ?? '').replace(/@.*$/, '').replace(/:\d+$/, '')
      : jid.replace(/@.*$/, '');

    const sender  = senderRaw.replace(/\D/g, '');
    const isOwner = ownerNumber && sender === ownerNumber;

    // Ignorer les messages fromMe qui NE viennent PAS du propriétaire
    // (évite les boucles sur les groupes, etc.)
    if (msg.key.fromMe && !isOwner) continue;
...
function attachHandlers(phoneNumber, sock, resolve, reject, onConnected) {
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      ...
      // onConnected en premier pour que le ping et ownerNumber soient prêts
  ...
  sock.ev.on('messages.upsert', (upsert) => {
    // 'notify'  = messages entrants d'autres contacts
    // 'append'  = messages envoyés depuis ce compte (chat avec soi-même)
